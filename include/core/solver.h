...
#define USING_SOLVER_MIXIN(type)          \
    ...
    using type::model
         ...
         */
        SolverMixin(std::vector<int> device_ids = {}, int num_sampler_per_worker = kAuto, size_t _gpu_memory_limit = kAuto) : gpu_memory_limit(_gpu_memory_limit), edge_table(-1), batch_id(0)
        {
...
// TODO:
        ...
        {
            // TODO: assignment of proc
            int temp_num_worker = num_worker;
#ifdef GRAPHVITE_WITH_MPI
            ...
            if (naive_parallel)
            {
                for (int i = 0; i < temp_num_worker; i++)
                    assignment[i] = {i, 0};
                return {assignment};
            }
            // fuhan: what is tied weights and what is this schedule?
            if (tied_weights)
                for (int x = 0; x < num_partition; x += temp_num_worker * 2)
                    for (int y = 0; y < num_partition; y += temp_num_worker * 2)
                    {
                        // diagonal
                        for (int i = 0; i < temp_num_worker; i++)
                        {
                        ...
                        for (int i = 0; i < temp_num_worker; i++)
                        {
                            int head_partition_id = x + temp_num_worker + i;
                            int tail_partition_id = y + temp_num_worker + i;
                            assignment[i] = {head_partition_id, tail_partition_id};
                        }
                        schedule.push_back(assignment);
                        for (int group_size = 1; group_size <= temp_num_worker; group_size *= 2)
                            for (int offset = 0; offset < group_size; offset++)
                            {
                                for (int i = 0; i < temp_num_worker; i++)
                                {
                                    int head_partition_id = x + (i / group_size * 2) * group_size + i % group_size;
                                    int tail_partition_id = y + (i / group_size * 2 + 1) * group_size + (i + offset) % group_size;
                                    assignment[i] = {head_partition_id, tail_partition_id};
                                }
                                schedule.push_back(assignment);
                                for (int i = 0; i < temp_num_worker; i++)
                ...
                for (int x = 0; x < num_partition; x += temp_num_worker)
                    for (int y = 0; y < num_partition; y += temp_num_worker)
                        for (int offset = 0; offset < temp_num_worker; offset++)
                        {
                            for (int i = 0; i < temp_num_worker; i++)
                            {
                                int head_partition_id = x + (i + offset) % temp_num_worker;
                                int tail_partition_id = y + i;
                                assignment[i] = {head_partition_id, tail_partition_id};
                            }
                            schedule.push_back(assignment);
...
#define USING_SAMPLER_MIXIN(type)    \
    ...
    using type::pool_size
         ...
         */
        SamplerMixin(Solver *_solver, int _device_id) : solver(_solver), device_id(_device_id), random(device_id)
        {
            CUDA_CHECK(cudaSetDevice(device_id));
...
#define USING_WORKER_MIXIN(type)     \
    ...
    using type::optimizer
         ...
         * @param _device_id GPU id
         */
        WorkerMixin(Solver *_solver, int _device_id) : solver(_solver), device_id(_device_id), negative_sampler(device_id),
...
// fuhan:make send buffer and recv buffer /
#ifdef PINNED_MEMORY
            CUDA_CHECK(cudaMallocHost(&send_buff, embedding.count * sizeof(Vector)));
#else
            ...
            if (protocol & kInPlace)
            {
                ...
                for (int i = 0; i < solver->size; i++)
                {
                    ...
                    MPI_Bcast(&recv_mapping_size, sizeof(size_t), MPI_BYTE, i, MPI_COMM_WORLD);
                    MPI_Bcast(&recv_embedding_size, sizeof(Index), MPI_BYTE, i, MPI_COMM_WORLD);
                    recv_mapping.resize(recv_mapping_size);
#ifdef PINNED_MEMORY

                    CUDA_CHECK(cudaMallocHost(&recv_buff, recv_embedding_size * sizeof(Vector)));
                    CUDA_CHECK(cudaMallocHost(&recv_mapping_buff, recv_mapping_size * sizeof(Index)));
#else

                    recv_buff = new char[recv_embedding_size * sizeof(Vector)];
                    ...
                    if (i == rank)
                    {
                        ...
                        memcpy(recv_buff, embedding.host_ptr, embedding.count * sizeof(Vector));
                    }
                    MPI_Bcast(recv_mapping_buff, recv_mapping_size * sizeof(Index), MPI_BYTE, i, MPI_COMM_WORLD);
                    MPI_Bcast(recv_buff, recv_embedding_size * sizeof(Vector), MPI_BYTE, i, MPI_COMM_WORLD);
                    ...
                    update_gradient.scatter(global_embedding, recv_mapping);
#ifdef PINNED_MEMORY
            ...
            if (solver->is_train && !(protocol & kGlobal))
            {
                ...
                for (int i = 0; i < num_moment; i++)
                {
                    ...
                    for (int k = 0; k < size; k++)
                    {
                        gradient_id = id;
                        recv_mapping_size = mapping->size();
                        recv_embedding_size = moment[i].count;
                        MPI_Bcast(&gradient_id, 1, MPI_INT, k, MPI_COMM_WORLD);
                        MPI_Bcast(&recv_mapping_size, sizeof(size_t), MPI_BYTE, k, MPI_COMM_WORLD);
                        MPI_Bcast(&recv_embedding_size, sizeof(Index), MPI_BYTE, k, MPI_COMM_WORLD);
                        recv_mapping.resize(recv_mapping_size);
#ifdef PINNED_MEMORY
                        CUDA_CHECK(cudaMallocHost(&recv_buff, recv_embedding_size * sizeof(Vector)));
                        CUDA_CHECK(cudaMallocHost(&recv_mapping_buff, recv_mapping_size * sizeof(Index)));
                        ...
                        if (i == solver->rank)
                        {
                            memcpy(recv_buff, send_buff, recv_embedding_size * sizeof(Vector));
                            memcpy(recv_mapping_buff, mapping->data(), mapping->size() * sizeof(Index));
                        }
                        ...
                        memcpy(recv_mapping.data(), recv_mapping_buff, recv_mapping.size() * sizeof(Index));
                        moment_gradient[i].copy(recv_buff, recv_embedding_size);
        ...
        void load_partition(int _head_partition_id, int _tail_partition_id)
        {
            ...
            sampler_hit = sampler_hit || ((sampler_protocol & kHeadPartition) && (sampler_protocol & kTailPartition) && head_partition_id == _tail_partition_id && _head_partition_id == tail_partition_id);
            sampler_hit = sampler_hit || ((sampler_protocol & (kHeadPartition | kTailPartition)) == kHeadPartition && head_partition_id == _head_partition_id);
            sampler_hit = sampler_hit || ((sampler_protocol & (kHeadPartition | kTailPartition)) == kTailPartition && tail_partition_id == _tail_partition_id);
        ...
        // fuhan: check sync for embeddings
        void check_sync(int id)
        {
            auto &global_embedding = *(solver->embeddings[id]);
            auto &global_moment = *(solver->moments[id]);
            ...
            for (const auto &embeddings : global_embedding)
            {
                ...
                CHECK(temp == embeddings) << "bad sync";
            }
        }
    };
